...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function afficherPokemons(page, pokeListeUse) {
      ...
      const tbody = document.getElementById('pokemonscartes');
      tbody.innerHTML = ''; 
 ...
 */      let x = 0;
        ...
        if (x >= debut && x< fin) {
          ...
          let nbType = 0;
          let typeAff = "";
          for (let typeCurrent in pokemon.types){
            if (nbType == 0) {

            
              tr.classList.add(pokemon.types[typeCurrent]); 

            }
            nbType++;
            if (typeAff !== "") {
              typeAff += " "; // Ajoute un espace si ce n'est pas le premier type
            }
            typeAff += `<div class="${pokemon.types[typeCurrent]}">${pokemon.types[typeCurrent]}</div>`;
          }
          console.log(typeAff);
          const imageNom = "../webp/images/" + pokemon.pokemon_id.toString().padStart(3, '0') + '.webp';
          tr.innerHTML = `
            <p class="idpokemon">${pokemon.pokemon_id}</p>
            <img src="${imageNom}" alt="${pokemon.nom}">
            <p class="nompokemon">  ${pokemon.pokemon_name} </p>
            <div class="type"> ${typeAff}</div>
            <p class="hover">plus de détails</p>
            `;
    ...
    if (i === 1) {
      btnPresed.style.backgroundColor = "#CCCCCC";
    }
  ...
  btnSuiv.addEventListener('click', function() {
      if (i < totalPages) {
          ...
          afficherPokemons(i, pokeListe);
          test();
          btnPresed.style.backgroundColor = "";
      }
      if (i === totalPages) {
        btnSuiv.style.backgroundColor = "#CCCCCC";
      }
  });
